...
foo <-f # save data
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ESTIMATES AFTER EXPONENTIATING
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
est
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#  MAKE THE BASELINE A VARIABLE, THE RESPONSE IS LOGGED IN THIS DATA
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 
  new <-   merge(df,setNames(subset(df, VISIT==1,select=c("ID", "y")),
                    c('ID', 'baseline')), by='ID')

# REMOVE VISIT 1 FROM THIS DATA
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #  FIT MODEL WITH BASELIBE AS COVARIATE, LOGED DATA HERE : mdata
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  fit.res <-  
   ...
   # (means1 <- emmeans(fit.res, "x", at = list(baseline = adjustment), data=mdata ))  # try on nlme, works but time consuming
    ...
    require(AICcmodavg)
    (predictSE.gls (fit.res, new, se.fit=T))
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  print(p1 + labs(y="Response", x = "Visit") +
          ggtitle(paste0("Individual responses ",
                         length(unique(dp$ID))," patients & modelled mean response with 95% CI from GLS model shown in black\nNumber of patient values at each time point") )
  )
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  est1$Approach <- "exp(log GLS data no baseline adj)"  # not using
  z0$Approach <- "exp(log GLS data no baseline adj)"
  z$Approach <- "exp(log GLS data with baseline adj)"
  
  
  dd <- rbind( musx, emus,  z, z0)
  ...
  pd1 <- ggplot(dd, aes(x= x, y=yhat, colour=Approach)) + 
    ...
    p1 <-  pd1 +geom_line(data=df, aes(x = VISIT, y = value, color = ID, group=ID),size=0.5,alpha=.2)  +
    theme(text = element_text(size=10),
          axis.text.x = element_text(angle = 0, vjust = 1, hjust=.5)) +
 ...
 # dplot <- merge(z,  df , by.x="x", by.y="VISIT", all=TRUE)
 ...
 # dplot <- dplot[complete.cases(dplot),]
  ...
  p1 <-  p +geom_line(data=dp, aes(x = VISIT, y = value, color = ID, group=ID),size=0.5,alpha=.2)  +
    ...
    # 
    # 
    # p <- ggplot(df, aes(x=VISIT, y= value, color=ID, group=ID)) + 
    # geom_line(size=0.5,alpha=.2) + theme(legend.position = "none") 
    # 
    # p1 <- p + geom_errorbar(data=zz,
    #                         aes(ymin=lower, ymax=upper ), color="black", width=.05, lwd=.2)
    # 
